...
abstract class BaseBindingActivity<VB : ViewBinding> : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (BaseConfig.AUTO_INIT_STATUS_BAR) {
            ...
            //读取一下此页面基础配置项是否为空
            getActivityBaseConfig().apply {
                if (this.windowBgColor != 0) {
                    //设置一个页面的底色，这边有个默认的值
                    window.decorView.setBackgroundColor(this.windowBgColor)
                }
                // 设置状态栏 字体颜色 和 背景
                if (null != this.textMode && this.textMode == PageUIConfig.StatusBarMode.DARK) {
                    // 设置状态栏为白底黑字
     ...
     *
     * */
    open fun getActivityBaseConfig(): PageUIConfig {
        return PageUIConfig()
    }
    ...
    suspend fun String.showToastSuspend() {
        withContext(Dispatchers.Main) {
            Toast.makeText(this@BaseBindingActivity, this@showToastSuspend, Toast.LENGTH_SHORT)
                .show()
        }
    }
